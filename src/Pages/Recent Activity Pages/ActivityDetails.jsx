    ...
    return (
      <div className="min-h-screen bg-white text-zinc-900 font-sans flex items-center justify-center">
        <div className="text-center p-12 border border-zinc-200 shadow-lg bg-[#fafafa]">
          <h2 className="text-2xl font-bold uppercase mb-4 tracking-widest text-zinc-800">
          ...
          <Link
            to="/dashboard"
            className="inline-flex items-center gap-2 px-8 py-4 bg-zinc-900 text-white font-mono-data uppercase text-xs hover:bg-blue-600 transition-colors shadow-md"
          >
    ...
    <div className="min-h-screen bg-[#fafafa] text-zinc-900 font-sans flex flex-col">
      ...
      <header className="h-16 border-b border-zinc-200 flex items-center justify-between px-6 bg-white sticky top-0 z-50 shadow-sm">
        <div className="flex items-center gap-4">
          ...
          <h1 className="text-sm font-bold uppercase tracking-widest text-zinc-800">
            Activity_Log_Viewer
            <span className="text-zinc-400 ml-2 font-mono-data font-normal">ID: {String(activity.id).padStart(4, '0')}</span>
          </h1>
        </div>
        <div className="hidden sm:flex items-center gap-3">
          <span className={`px-2 py-1 text-[10px] uppercase font-bold font-mono-data border ${activity.priority === "High" ? "bg-red-50 text-red-600 border-red-100" :
              activity.priority === "Medium" ? "bg-orange-50 text-orange-600 border-orange-100" :
                "bg-green-50 text-green-600 border-green-100"
            }`}>
            PRIORITY: {activity.priority}
          </span>
          <span className="px-2 py-1 text-[10px] uppercase font-bold font-mono-data bg-zinc-100 text-zinc-500 border border-zinc-200">
            STATUS: ARCHIVED
          </span>
        </div>
      </header>
        ...
        <div className="border border-zinc-200 bg-white shadow-sm">
          ...
          <div className="flex flex-col md:flex-row border-b border-zinc-200">
            <div className="flex-1 p-8 border-b md:border-b-0 md:border-r border-zinc-200">
              <span className="text-[10px] uppercase text-zinc-400 font-bold tracking-widest mb-3 block">User Identity</span>
              <div className="flex items-center gap-4">
                ...
                <div>
                  <p className="font-bold text-lg uppercase text-zinc-900 leading-tight">{activity.user}</p>
                  <p className="text-xs font-mono-data text-zinc-500 mt-1">AUTH_LEVEL: ADMIN</p>
                </div>
              </div>
            </div>

            <div className="flex-1 p-8 flex flex-col justify-center">
              <span className="text-[10px] uppercase text-zinc-400 font-bold tracking-widest mb-3 block">Timestamp</span>
          ...
          <div className="p-10 md:p-14 bg-zinc-50/50">
            <div className="mb-10">
              <span className="text-[10px] uppercase text-zinc-400 font-bold tracking-widest mb-3 block">Action Executed</span>
              <h2 className="text-3xl md:text-5xl font-black uppercase tracking-tight leading-[0.9] text-zinc-900 mb-6">
                {activity.action}
              </h2>
              <div className="inline-flex items-center gap-2 px-4 py-2 bg-white border border-zinc-200 shadow-sm">
                ...
                <span className="text-xs font-bold uppercase text-zinc-700">{activity.category}</span>
              </div>
            </div>

            <div className="border border-zinc-200 bg-white p-8 relative shadow-sm hover:border-blue-200 transition-colors">
              <span className="absolute -top-3 left-6 bg-white px-2 text-[10px] uppercase font-bold text-zinc-400 tracking-widest border border-zinc-100 shadow-sm">
                System Payload
              </span>
          ...
          <div className="bg-zinc-50 border-t border-zinc-200 p-6 flex flex-wrap gap-4">
            <button className="flex-1 sm:flex-none flex items-center justify-center gap-2 px-6 py-3 bg-zinc-900 text-white text-xs font-bold uppercase hover:bg-blue-600 transition-colors shadow-md">
              <Download className="w-4 h-4" />
              Export Json
            </button>
            <button className="flex-1 sm:flex-none flex items-center justify-center gap-2 px-6 py-3 border border-zinc-300 bg-white text-zinc-900 text-xs font-bold uppercase hover:bg-zinc-100 transition-colors shadow-sm">
